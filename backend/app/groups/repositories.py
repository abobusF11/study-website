...
class GroupRepository:
    ...
    async def create_group(db: AsyncSession, group_data: GroupCreate) -> dict:
        """
        Создает новую группу с курсами, клиентами и преподавателями
        Возвращает словарь с ID созданной группы
        """
    ...
    # ) -> List[GroupResponse]:
        ...
        """
        Получает список групп с возможностью фильтрации по дате
        Возвращает список GroupResponse с полной информацией о группах
        """
        # Создаем базовый запрос
        query = select(Group).where(Group.isOrder == is_order)
    ...
    @staticmethod
    async def update_group(db: AsyncSession, group_update: GroupUpdate) -> dict:
        """
        Обновляет информацию о группе, включая курсы, клиентов и преподавателей
        Возвращает словарь с ID обновленной группы
        """
        try:
